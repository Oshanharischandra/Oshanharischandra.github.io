...
export default function Certifications() {
  ...
  const nextSlide = useCallback(() => {
    setCarouselIndex((prev) => (prev + cardsPerView >= certifications.length ? 0 : prev + cardsPerView));
  }, [cardsPerView]);

  const prevSlide = () => {
    setCarouselIndex((prev) => (prev - cardsPerView < 0 ? Math.max(0, certifications.length - cardsPerView) : prev - cardsPerView));
  };

  // Auto-advance
  useEffect(() => {
    if (prefersReducedMotion || isHovering || showGallery) return;
    
    const timer = setInterval(() => {
      nextSlide();
    }, 3000);
    return () => clearInterval(timer);
  }, [nextSlide, prefersReducedMotion, isHovering, showGallery]);
